...
ipcMain.handle('stop-recording-stream', async (event, format) => {
    ...
    if (!tempInput)
        return { success: false, error: 'No temp file path' };
    // Prompt save dialog
                ...
                await new Promise((resolve, reject) => {
                    ...
                    if (useStreamCopy) {
                        ...
                        if (videoCodec === 'h264') {
                            ...
                            outputOptions.push('-c:v', 'libx264', '-preset', preset, // medium/slow - 最优质量
                        ...
                        if (audioCodec === 'aac' || audioCodec === 'mp3') {
                            ...
                            outputOptions.push('-c:a', 'aac', '-b:a', '192k', // 提高音频比特率到 192kbps（从 128k 提高到 192k，更高质量）
...
function createPreviewWindow(imagePath) {
    if (previewWindow) {
        previewWindow.close();
    }
...
ipcMain.handle('capture-selection', async (_event, bounds) => {
...
ipcMain.handle('copy-to-clipboard', async (_event, imageData) => {
    try {
        const base64Data = imageData.replace(/^data:image\/\w+;base64,/, '');
        const buffer = Buffer.from(base64Data, 'base64');
...
ipcMain.handle('show-preview', (_event, imageData) => {
...
ipcMain.handle('show-recording-overlay', (_event, bounds) => {
    try {
...
async function captureArea() {
    return new Promise((resolve) => {
        createSelectorWindow();
        selectorWindow?.webContents.once('ipc-message', (_event, channel, bounds) => {
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            desktopCapturer.getSources({
                ...
                const primarySource = sources.find((s) => s.id.startsWith('screen:0')) || sources[0];
